...
def add_rain_map_layer(m):
    latitude_north = -18.23
    longitude_west = -54.33
    longitude_east = -40.52
    latitude_south = -28.46
    
    rain_image_folder = "tp_images"
    rain_image_files = sorted([file for file in os.listdir(rain_image_folder) if file.endswith(".png")])
    ...
    rain_layer_control = folium.LayerControl(collapsed=True, name='Rain Data')
    for rain_image_file in rain_image_files:
        rain_image_path = os.path.join(rain_image_folder, rain_image_file)
        add_image_to_map(rain_image_path, rain_image_file, rain_layer_control)

    m.add_child(rain_layer_control)

    html = """
                <div class="floating-icon">
                    <a href="mapa_vento.html" target="_blank"><i class="fa fa-info-circle fa-2x" style="color:green;"></i></a>
                </div>
            """
...
def rain_map_gen(dados_coordenada, dados_ocorrencia):
    ...
    add_rain_map_layer(m)

    m.save('mapa_chuva.html')
